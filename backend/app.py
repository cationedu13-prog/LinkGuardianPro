...
def hash_password(password: str) -> str:
    """Bcrypt password hash (safe for production)."""
    return bcrypt.hashpw(password.encode(), bcrypt.gensalt()).decode()


def verify_password(password: str, stored_hash: str) -> bool:
    """
    Verify password. Supports both bcrypt (new) and legacy SHA-256 (old users).
    On login with a legacy SHA-256 hash, the route re-hashes with bcrypt automatically.
    """
    if stored_hash.startswith("$2b$") or stored_hash.startswith("$2a$"):
        return bcrypt.checkpw(password.encode(), stored_hash.encode())
    # Legacy SHA-256 — will be upgraded to bcrypt on next successful login
...
def api_login():
    ...
    # Auto-upgrade legacy SHA-256 hash → bcrypt on first successful login
...
def api_forgot_password():
    ...
    2. Generate a secure token, store its hash + expiry in DB
    3. Send reset link via SMTP email
    ...
    # Always return 200 so we don't reveal whether email exists (security)
    success_msg = {"message": "If that email is registered, a reset link has been sent."}

    try:
        res = (supabase.table("users")
               .select("id, full_name").eq("email", email).execute())
        if not res.data:
        ...
        ok = send_email(email, "Reset your LinkGuardian Pro password", html_body)
        if not ok:
        ...
        return jsonify(success_msg), 200   # never expose internals


# ─── RESET PASSWORD (token verification + new password) ──────────────────────
